    ...
    try:
        ...
        with pool.connection() as conn:
            ...
            for b in bookings:
                ...
                print(
                    f"{status_emoji} #{b['booking_id']} | {b['status']:12} | {b['title'] or 'N/A'} | code: {b['booking_code']} | {b['created_at']}"
                )
            ...
            # Check user exists
            cursor.execute(
                "SELECT user_id, first_name, username, phone, city FROM users WHERE user_id = %s",
                (user_id,),
            )
            ...
            if user:
                print(
                    f"ID: {user['user_id']}, Name: {user['first_name']}, Username: {user['username']}, Phone: {user['phone']}, City: {user['city']}"
                )
            else:
