...
async def main():
    ...
    store_id = db.add_store(owner_id=seller_id, name='Test Store', city='Ташкент', address='Some address', description='', category='Ресторан', phone='+998900000000')
    offer_id = db.add_offer(store_id=store_id, title='Test Offer', description='Nice', original_price=10000.0, discount_price=5000.0, quantity=3, available_from='10:00', available_until='22:00')
    ...
    cb_message = Message(message_id=1, date=datetime.now(), chat=chat, from_user=tg_user, text='Hot offers')
    cbq = CallbackQuery(id='cbq_1', from_user=tg_user, chat_instance='ci_1', data=f'book_{offer_id}', message=cb_message)
