...
def check_callback_handlers(file_path: str):
    ...
    for match in callbacks:
        ...
        if "state: FSMContext" in func_code:
            if (
                "await state.clear()" not in func_code
                and "await state.set_state(" not in func_code
                and "await state.update_data(" not in func_code
            ):
                issues.append(
                    {
                        "type": "unused_state",
...
def print_report(issues, callback_count):
    ...
    print(f"\n{'='*80}")
    print("📊 ОТЧЁТ О ПРОВЕРКЕ CALLBACK ОБРАБОТЧИКОВ")
    print(f"{'='*80}\n")
...
def generate_fix_script(issues, output_file="fix_callbacks.txt"):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for issue in sorted(high_priority, key=lambda x: x["line"]):
            ...
            f.write(
                "Исправление: Добавить 'await callback.answer()' в конце функции или перед return\n"
            )
            f.write("-" * 80 + "\n")
