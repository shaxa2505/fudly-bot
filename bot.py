...
from handlers.common import (
    ...
    ChangeCity, EditOffer, ConfirmOrder, BookOffer, BrowseOffers, OrderDelivery,
...
@dp.message(F.text.contains("🔥 Горячее") | F.text.contains("🔥 Issiq"))
async def hot_offers_handler(message: types.Message, state: FSMContext):
    """Показать топ-20 горячих предложений с лучшими скидками"""
    lang = db.get_user_language(message.from_user.id)
    user = db.get_user(message.from_user.id)
    
    logger.info(f"🔥 Hot offers requested by user {message.from_user.id}")
    ...
    if not offers:
        ...
        await message.answer(
            f"😔 {get_text(lang, 'no_offers')}\n\n{no_offers_msg}"
        )
        return
    
    # Сохраняем список предложений в FSM state
    await state.set_state(BrowseOffers.offer_list)
    await state.update_data(offer_list=[offer[0] for offer in offers])  # Сохраняем только offer_id
    
    # Формируем компактный список с нумерацией
    text = f"{get_text(lang, 'hot_offers_title')}\n"
    text += f"📍 {city}\n"
    text += f"{get_text(lang, 'hot_offers_subtitle')}\n"
    text += f"Показано: {len(offers)} из {total_count}\n\n"
    
    for idx, offer in enumerate(offers, 1):
        try:
            # [0]=offer_id, [2]=title, [4]=original_price, [5]=discount_price, [15]=store_name, [19]=discount_percent
            ...
            logger.error(f"Error formatting offer {idx}: {e}, offer length: {len(offer)}")
            continue
    
    text += f"\n{get_text(lang, 'select_by_number')}"
    
    # Добавляем кнопку "Показать ещё", если есть ещё товары
    builder = InlineKeyboardBuilder()
    ...
    try:
        await message.answer(text, parse_mode="HTML", reply_markup=builder.as_markup() if builder.export() else None)
        logger.info(f"✅ Sent hot offers list to user {message.from_user.id}")
    except Exception as e:
        logger.error(f"❌ Failed to send hot offers: {e}")
        await message.answer(f"😔 {get_text(lang, 'error')}")


@dp.callback_query(F.data.startswith("hot_offers_next_"))
async def hot_offers_pagination(callback: types.CallbackQuery, state: FSMContext):
    """Пагинация списка горячих предложений"""
    try:
        ...
        if not offers:
            await callback.answer("Больше предложений нет" if lang == 'ru' else "Boshqa takliflar yo'q", show_alert=True)
            return
        
        # Обновляем список в FSM - добавляем новые offer_id
        data = await state.get_data()
        existing_list = data.get('offer_list', [])
        new_ids = [offer[0] for offer in offers]
        # Добавляем только те, которых ещё нет (избегаем дубликатов)
        for offer_id in new_ids:
            if offer_id not in existing_list:
                existing_list.append(offer_id)
        await state.update_data(offer_list=existing_list)
        logger.info(f"📝 Updated offer_list: now {len(existing_list)} offers in state")
        
        # Формируем текст со следующими товарами
        text = f"{get_text(lang, 'hot_offers_title')}\n"
        text += f"📍 {city}\n"
        text += f"{get_text(lang, 'hot_offers_subtitle')}\n"
        text += f"Показано: {offset + len(offers)} из {total_count}\n\n"
        ...
        for idx, offer in enumerate(offers, offset + 1):
            try:
                ...
                logger.error(f"Error formatting offer {idx}: {e}")
                continue
        ...
        builder = InlineKeyboardBuilder()
        if offset + len(offers) < total_count:
            next_offset = offset + 20
            next_text = "➡️ Показать ещё 20" if lang == 'ru' else "➡️ Yana 20 ta ko'rsatish"
            builder.button(text=next_text, callback_data=f"hot_offers_next_{next_offset}")
        builder.adjust(1)
        
        await callback.message.answer(text, parse_mode="HTML", reply_markup=builder.as_markup() if builder.export() else None)
        await callback.answer()
        
    except Exception as e:
        logger.error(f"hot_offers_pagination error: {e}")
        await callback.answer("Ошибка" if db.get_user_language(callback.from_user.id) == 'ru' else "Xato", show_alert=True)

# ============== МЕСТА (ФИЛЬТР ПО ТИПУ ЗАВЕДЕНИЙ) ==============

@dp.message(F.text.contains("🏪 Места") | F.text.contains("🏪 Joylar"))
async def browse_places_handler(message: types.Message):
    """Выбор типа заведения для просмотра предложений"""
    lang = db.get_user_language(message.from_user.id)
...
async def business_type_selected(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик выбора типа заведения - показываем список магазинов"""
    lang = db.get_user_language(callback.from_user.id)
    user = db.get_user(callback.from_user.id)
    ...
    db_type = type_map.get(business_type, 'supermarket')
    ...
    if not stores:
        ...
        type_name = type_names.get(business_type, business_type)
        no_stores_msg = "В этой категории пока нет магазинов с активными предложениями" if lang == 'ru' else "Bu kategoriyada hali do'konlar yo'q"
    ...
    emoji = type_emoji.get(business_type, '🏪')
    ...
    text = f"{emoji} <b>{type_names.get(business_type, business_type.upper())}</b>\n"
    text += f"📍 {city}\n\n"
    ...
    for idx, store in enumerate(stores, 1):
        ...
        text += f"   ⭐ {avg_rating:.1f}/5 ({ratings_count})\n"
        text += f"   🔥 {'Предложений' if lang == 'ru' else 'Takliflar'}: {offers_count}\n\n"
    
    select_msg = "💬 Введите номер магазина для просмотра" if lang == 'ru' else "💬 Do'kon raqamini kiriting"
    text += f"\n{select_msg}"
...
# Обработчик выбора магазина по номеру (из списка "Места")
...
async def select_store_by_number(message: types.Message, state: FSMContext):
    """Обработка выбора магазина по номеру из списка Места"""
    lang = db.get_user_language(message.from_user.id)
    
    logger.info(f"🏪 User {message.from_user.id} selected store number: {message.text}")
    
    # Получаем список магазинов из state
    data = await state.get_data()
    store_list = data.get('store_list', [])
    
    logger.info(f"📋 Current store_list length: {len(store_list)}")
    ...
    try:
        ...
        error_msg = "Пожалуйста, введите корректный номер магазина" if lang == 'ru' else "Iltimos, to'g'ri raqam kiriting"
        await message.answer(f"❌ {error_msg}")
        return
    
    # Проверяем диапазон
    if number < 1 or number > len(store_list):
        range_msg = f"Номер должен быть от 1 до {len(store_list)}" if lang == 'ru' else f"Raqam 1 dan {len(store_list)} gacha bo'lishi kerak"
        await message.answer(f"❌ {range_msg}")
        return
    ...
    # Показываем карточку магазина с кнопками
    await show_store_card(message, store_id, lang)


async def show_store_card(message: types.Message, store_id: int, lang: str):
    """Показать карточку магазина с кнопками действий"""
    logger.info(f"🏪 Showing store card for store_id={store_id}")
    ...
    text = f"{emoji} <b>{store_name}</b>\n\n"
    text += f"🏷 {type_name}\n"
    text += f"📍 {store_address}, {store_city}\n"
    if store_description:
        text += f"📝 {store_description}\n"
    text += f"📞 {store_phone}\n"
    text += f"⭐ {'Рейтинг' if lang == 'ru' else 'Reyting'}: {avg_rating:.1f}/5 ({ratings_count} {'отзывов' if lang == 'ru' else 'sharh'})\n"
    text += f"🔥 {'Горячих предложений' if lang == 'ru' else 'Issiq takliflar'}: {offers_count}"
    
    # Создаём кнопки
    builder = InlineKeyboardBuilder()
    builder.button(
        text=f"🛍 {'Все товары' if lang == 'ru' else 'Barcha mahsulotlar'} ({offers_count})",
        callback_data=f"store_offers_{store_id}"
    )
    builder.button(
        text=f"⭐ {'Отзывы' if lang == 'ru' else 'Sharhlar'} ({ratings_count})",
        callback_data=f"store_reviews_{store_id}"
    )
    builder.button(
        text="◀️ " + ("Назад" if lang == 'ru' else "Orqaga"),
...
# Обработчик выбора предложения по номеру
...
async def select_offer_by_number(message: types.Message, state: FSMContext):
    """Обработка выбора предложения по номеру из списка"""
    lang = db.get_user_language(message.from_user.id)
    
    logger.info(f"🔢 User {message.from_user.id} selected number: {message.text}")
    
    # Получаем список предложений из state
    data = await state.get_data()
    offer_list = data.get('offer_list', [])
    
    logger.info(f"📋 Current offer_list length: {len(offer_list)}")
    ...
    try:
        ...
        error_msg = "Пожалуйста, введите корректный номер товара" if lang == 'ru' else "Iltimos, to'g'ri raqam kiriting"
        await message.answer(f"❌ {error_msg}")
        return
    
    # Проверяем диапазон
    if number < 1 or number > len(offer_list):
        range_msg = f"Номер должен быть от 1 до {len(offer_list)}" if lang == 'ru' else f"Raqam 1 dan {len(offer_list)} gacha bo'lishi kerak"
        await message.answer(f"❌ {range_msg}")
        return
    ...
    # Показываем детали предложения
    offer_data = db.get_offer(offer_id)
    ...
    # Очищаем state после выбора
    await state.clear()
    
    # Показываем детальную карточку предложения (используем существующую функцию)
...
async def show_offer_details(message: types.Message, offer_data: tuple, lang: str):
    ...
    discount_percent = round(((original_price - discount_price) / original_price) * 100, 1)
    
    # Форматируем дату
    try:
        exp_date = datetime.strptime(expiry_date, "%Y-%m-%d %H:%M:%S").strftime("%d.%m.%Y")
    except:
        exp_date = expiry_date[:10] if expiry_date else "Не указано"
    
    # Переводим категорию
    category_names = {
        'dairy': '🥛 Молочное' if lang == 'ru' else '🥛 Sut mahsulotlari',
        'bakery': '🥖 Выпечка' if lang == 'ru' else '🥖 Non mahsulotlari',
        'meat': '🥩 Мясное' if lang == 'ru' else '🥩 Go\'sht',
        'fruits': '🍎 Фрукты' if lang == 'ru' else '🍎 Mevalar',
        'vegetables': '🥬 Овощи' if lang == 'ru' else '🥬 Sabzavotlar',
        'ready_food': '🍱 Готовая еда' if lang == 'ru' else '🍱 Tayyor taom',
    ...
    if description:
        text += f"{description}\n\n"
    text += f"💰 {'Обычная цена' if lang == 'ru' else 'Oddiy narx'}: <s>{original_price:,.0f} сум</s>\n"
    text += f"💵 {'Цена со скидкой' if lang == 'ru' else 'Chegirmadagi narx'}: <b>{discount_price:,.0f} сум</b>\n"
    text += f"📊 {'Скидка' if lang == 'ru' else 'Chegirma'}: <b>-{discount_percent:.0f}%</b>\n\n"
    text += f"🏪 {store_name}\n"
    text += f"📍 {store_address}, {store_city}\n"
    text += f"🏷 {category_text}\n"
    text += f"📦 {'Доступно' if lang == 'ru' else 'Mavjud'}: {quantity} {unit}\n"
    text += f"⏰ {'Годен до' if lang == 'ru' else 'Yaroqlilik muddati'}: {exp_date}"
    
    # Добавляем информацию о доставке если включена
    if delivery_enabled:
        currency_ru = 'сум'
        currency_uz = "so'm"
        text += f"\n\n🚚 {'Доставка' if lang == 'ru' else 'Yetkazib berish'}: {delivery_price:,.0f} {currency_ru if lang == 'ru' else currency_uz}"
        if min_order_amount > 0:
            text += f"\n{'Минимальная сумма заказа' if lang == 'ru' else 'Minimal buyurtma summasi'}: {min_order_amount:,.0f} {currency_ru if lang == 'ru' else currency_uz}"
    
    # Кнопки
    builder = InlineKeyboardBuilder()
    builder.button(
        text="🛒 " + ("Забронировать (самовывоз)" if lang == 'ru' else "Bron qilish (olib ketish)"), 
        callback_data=f"book_{offer_id}"
    )
    
    # Добавляем кнопку доставки если включена
    if delivery_enabled:
        builder.button(
            text="🚚 " + ("Заказать с доставкой" if lang == 'ru' else "Yetkazib berish bilan"),
            callback_data=f"order_delivery_{offer_id}"
        )
    
    builder.button(
        text="🏪 " + ("О магазине" if lang == 'ru' else "Dokon haqida"),
    ...
    if photo:
        try:
            ...
            logger.error(f"Error sending photo for offer {offer_id}: {e}")
            await message.answer(text, parse_mode="HTML", reply_markup=builder.as_markup())
    else:
        await message.answer(text, parse_mode="HTML", reply_markup=builder.as_markup())
...
async def show_store_offers(callback: types.CallbackQuery, state: FSMContext):
    """Показать все товары магазина"""
    lang = db.get_user_language(callback.from_user.id)
    store_id = int(callback.data.split("_")[-1])
    
    store = db.get_store(store_id)
    if not store:
        await callback.answer("Магазин не найден" if lang == 'ru' else "Do'kon topilmadi", show_alert=True)
        return
    
    # Получаем все активные товары магазина
    offers = db.get_offers_by_store(store_id)
    
    if not offers:
        await callback.answer("В магазине пока нет активных предложений" if lang == 'ru' else "Do'konda hali takliflar yo'q", show_alert=True)
        return
    ...
    builder.button(
        text="◀️ " + ("К магазину" if lang == 'ru' else "Do'konga qaytish"),
        callback_data=f"back_to_store_{store_id}"
    )
    builder.adjust(1)
...
async def store_offers_pagination(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Парсим callback_data: store_offers_next_{store_id}_{offset}
    ...
    all_offers = db.get_offers_by_store(store_id)
    
    if offset >= len(all_offers):
        await callback.answer("Больше товаров нет" if lang == 'ru' else "Boshqa mahsulotlar yo'q", show_alert=True)
        return
    
    # Берём следующие 20 товаров
    offers = all_offers[offset:offset+20]
    
    # Обновляем список в state
    data = await state.get_data()
    existing_list = data.get('offer_list', [])
    for offer in offers:
        if offer[0] not in existing_list:
            existing_list.append(offer[0])
    await state.update_data(offer_list=existing_list)
    ...
    text = f"🛍 <b>{store_name}</b>\n"
    text += f"{'Все товары' if lang == 'ru' else 'Barcha mahsulotlar'}\n"
    text += f"Показано: {offset + len(offers)} из {len(all_offers)}\n\n"
    ...
    # Кнопки пагинации
    builder = InlineKeyboardBuilder()
    if offset + len(offers) < len(all_offers):
        next_offset = offset + 20
        builder.button(
            text="➡️ " + ("Показать ещё 20" if lang == 'ru' else "Yana 20 ta"),
            callback_data=f"store_offers_next_{store_id}_{next_offset}"
        )
    builder.button(
        text="◀️ " + ("К магазину" if lang == 'ru' else "Do'konga qaytish"),
        callback_data=f"back_to_store_{store_id}"
...
async def show_store_reviews(callback: types.CallbackQuery):
    ...
    # Получаем отзывы
    reviews = db.get_store_ratings(store_id)
    avg_rating = db.get_store_average_rating(store_id)
    
    store_name = store[2]
    text = f"⭐ <b>{store_name}</b>\n"
    text += f"{'Отзывы' if lang == 'ru' else 'Sharhlar'}\n"
    
    avg_text = 'Средний рейтинг' if lang == 'ru' else "O'rtacha reyting"
    text += f"{avg_text}: {avg_rating:.1f}/5 ({len(reviews)})\n\n"
    
    if not reviews:
        text += "😔 " + ("Отзывов пока нет" if lang == 'ru' else "Hali sharhlar yo'q")
    else:
        # Показываем последние 5 отзывов
        for review in reviews[:5]:
            # review: [0]rating_id, [1]store_id, [2]user_id, [3]rating, [4]comment, [5]created_at
            rating = review[3]
            comment = review[4] if len(review) > 4 and review[4] else ""
...
async def back_to_store_card(callback: types.CallbackQuery):
    """Вернуться к карточке магазина"""
    store_id = int(callback.data.split("_")[-1])
    lang = db.get_user_language(callback.from_user.id)
    
    # Показываем карточку магазина снова
    await show_store_card(callback.message, store_id, lang)
    await callback.answer()
...
async def select_offer(callback: types.CallbackQuery):
    ...
    try:
        ...
        if len(offer) > 11 and offer[11] and str(offer[11]).strip():
            ...
            logger.info(f"No photo to display for offer {offer_id}")
...
@dp.callback_query(F.data.startswith("order_delivery_"))
async def order_delivery_start(callback: types.CallbackQuery, state: FSMContext):
    """Начало заказа с доставкой - спрашиваем количество"""
    lang = db.get_user_language(callback.from_user.id)
    ...
    # Проверяем что доставка включена для этого магазина
    store = db.get_store(offer[1])
    if not store:
        await callback.answer("❌ Магазин не найден", show_alert=True)
        return
    
    delivery_enabled = store[-3] if len(store) >= 3 else 0
    ...
    await state.update_data(offer_id=offer_id, store_id=offer[1])
    ...
    await callback.message.answer(
        f"🍽 <b>{offer[2]}</b>\n\n"
        f"📦 {'Доступно' if lang == 'ru' else 'Mavjud'}: {offer[6]} {unit_ru if lang == 'ru' else unit_uz}\n"
        f"💰 {'Цена за 1 шт' if lang == 'ru' else '1 dona narxi'}: {int(offer[5]):,} {currency_ru if lang == 'ru' else currency_uz}\n\n"
        f"{'Сколько вы хотите заказать?' if lang == 'ru' else 'Nechta buyurtma qilasiz?'} (1-{offer[6]})",
        parse_mode="HTML",
        reply_markup=cancel_keyboard(lang)
    )
    await callback.answer()

@dp.message(OrderDelivery.quantity)
async def order_delivery_quantity(message: types.Message, state: FSMContext):
    """Обработка количества и запрос адреса доставки"""
    lang = db.get_user_language(message.from_user.id)
    ...
    try:
        ...
        if not offer or offer[6] < quantity:
            await message.answer(f"❌ {'Доступно только' if lang == 'ru' else 'Faqat mavjud'} {offer[6]} {'шт' if lang == 'ru' else 'dona'}")
            return
        
        # Проверяем минимальную сумму заказа
        store = db.get_store(store_id)
        min_order_amount = store[-1] if len(store) >= 1 else 0
        order_amount = offer[5] * quantity
...
@dp.message(OrderDelivery.address)
async def order_delivery_address(message: types.Message, state: FSMContext):
    """Обработка адреса доставки и запрос способа оплаты"""
    lang = db.get_user_language(message.from_user.id)
    
    address = message.text.strip()
    if len(address) < 10:
        await message.answer(
            "❌ " + ("Пожалуйста, укажите полный адрес доставки" if lang == 'ru' else "Iltimos, to'liq manzilni kiriting")
        )
        return
...
async def order_payment_proof(message: types.Message, state: FSMContext):
    ...
    if not order_id:
        await message.answer("❌ " + ("Ошибка создания заказа" if lang == 'ru' else "Buyurtma yaratishda xatolik"))
        await state.clear()
        return
    ...
    payment_ru = 'Оплата'
    payment_uz = "To'lov"
    card_transfer_ru = 'Перевод на карту'
    card_transfer_uz = "Kartaga o'tkazma"
    screenshot_text = "To'lov skrinsho yuqorida"
    
    notification_kb = InlineKeyboardBuilder()
    notification_kb.button(text="✅ " + ("Подтвердить оплату" if lang == 'ru' else "To'lovni tasdiqlash"), callback_data=f"confirm_payment_{order_id}")
    ...
    card_transfer_ru = 'Перевод на карту'
    card_transfer_uz = "Kartaga o'tkazma"
    wait_confirm_ru = 'Ожидайте подтверждения оплаты от магазина'
    wait_confirm_uz = "Do'kondan to'lovni tasdiqlashni kuting"
    await message.answer(
        f"✅ <b>{'Заказ оформлен!' if lang == 'ru' else 'Buyurtma qabul qilindi!'}</b>\n\n"
        f"📦 {'Заказ' if lang == 'ru' else 'Buyurtma'} #{order_id}\n"
        f"🍽 {offer_title}\n"
        ...
        f"💰 {payment_ru if lang == 'ru' else payment_uz}: {card_transfer_ru if lang == 'ru' else card_transfer_uz}\n"
        f"💵 {'Итого' if lang == 'ru' else 'Jami'}: <b>{total_amount:,} {currency_ru if lang == 'ru' else currency_uz}</b>\n\n"
        f"{wait_confirm_ru if lang == 'ru' else wait_confirm_uz}",
        parse_mode="HTML"
    )
...
async def offer_details(callback: types.CallbackQuery):
    ...
    # [12]created_at, [13]unit, [14]category
    ...
    # Показываем срок годности если есть (expiry_date на позиции 9, status на позиции 10)
    if len(offer_data) > 9 and offer_data[10] == 'active' and offer_data[9]:
        time_remaining = db.get_time_remaining(offer_data[9])
        if time_remaining:
            text += f"{time_remaining}\n"
    
    # АКТУАЛЬНЫЕ индексы после ALTER TABLE и JOIN:
    # [0-14] поля offers (включая unit, category)
    # [15]store_name, [16]address, [17]city, [18]phone, [19]store_desc
    text += f"\n🏪 <b>Магазин:</b> {offer_data[15]}\n"
    text += f"📍 <b>Адрес:</b> {offer_data[16]}, {offer_data[17]}\n"
    
    if len(offer_data) > 18 and offer_data[18]:  # phone
        text += f"📞 <b>Телефон:</b> {offer_data[18]}\n"
    
    if len(offer_data) > 19 and offer_data[19]:  # store description
    ...
    logger.info(f"Offer_details {offer_id}: len={len(offer_data)}, photo=[11]={offer_data[11] if len(offer_data) > 11 else 'N/A'}")
    if len(offer_data) > 11 and offer_data[11] and str(offer_data[11]).strip():
        try:
            logger.info(f"Trying to edit_media with photo: {offer_data[11][:50]}...")
            # Пытаемся сначала отредактировать медиа (если сообщение уже с фото)
            ...
            logger.info("Media edited successfully")
        except Exception as e:
            logger.error(f"Error editing media, trying delete and send new: {e}")
            try:
                # Если не удалось отредактировать, удаляем старое и отправляем новое с фото
                await callback.message.delete()
                await callback.message.answer_photo(
                    photo=offer_data[11],
                    caption=text,
                ...
                logger.info("Photo sent successfully after delete")
            except Exception as e2:
                logger.error(f"Error sending photo: {e2}", exc_info=True)
        ...
        logger.info(f"No photo to display for offer_details {offer_id}")
...
@dp.message(F.text.contains("Мои бронирования") | F.text.contains("Mening buyurt") | F.text.contains("🛒 Корзина") | F.text.contains("🛒 Savat"))
...
@dp.callback_query(F.data.startswith("cancel_order_customer_"))
async def cancel_order_customer(callback: types.CallbackQuery):
    ...
    if order[1] != callback.from_user.id:
        await callback.answer("❌ " + ("Это не ваш заказ" if lang == 'ru' else "Bu sizning buyurtmangiz emas"), show_alert=True)
        return
    
    # Можно отменить только pending и confirmed
    if order[10] not in ['pending', 'confirmed']:
        await callback.answer("❌ " + ("Заказ нельзя отменить" if lang == 'ru' else "Buyurtmani bekor qilib bo'lmaydi"), show_alert=True)
        return
    ...
    # Уведомляем продавца
    store = db.get_store(order[2])  # store_id
    if store:
        seller_lang = db.get_user_language(store[1])
        try:
            await bot.send_message(
                store[1],
            ...
            logger.error(f"Failed to notify seller {store[1]}: {e}")
...
async def create_offer_prices_and_quantity(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Сохраняем (сохраняем все предыдущие данные включая photo)
        await state.update_data(
            ...
            # photo и другие данные уже в state, не затираем их
        )
...
async def select_expiry_simple(callback: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"Creating offer: store_id={data.get('store_id')}, title={data.get('title')}, category={category}, photo={photo}")
...
async def create_offer_time_until(message: types.Message, state: FSMContext):
    ...
    print("📸 data['photo'] =", data.get('photo'))
    if data.get('photo'):
        await message.answer_photo(
            photo=data['photo'],
            caption=text,
...
async def bulk_create_start(message: types.Message, state: FSMContext):
    ...
    if len(stores) == 1:
        ...
        await state.set_state(BulkCreate.titles)
    else:
        # Несколько магазинов - нужно выбрать
        await message.answer(
            get_text(lang, 'choose_store'),
...
@dp.message(F.text.contains("Профиль") | F.text.contains("Profil") | F.text.contains("⚙️ Профиль") | F.text.contains("⚙️ Profil"))
async def profile(message: types.Message):
    ...
    if (get_user_field(user, 'role', 'customer') == 'customer') or user_view_mode.get(message.from_user.id) == 'customer':
        ...
        active_orders = len([o for o in orders if o[10] in ['pending', 'confirmed', 'preparing', 'delivering']])
    ...
    # Добавляем статистику для продавцов
        ...
        if stores:
            ...
            for store in stores:
                ...
                store_orders = db.get_user_orders(message.from_user.id)  # Нужно будет использовать get_store_orders
                
                completed_bookings = [b for b in store_bookings if b[3] == 'completed']
                total_bookings += len(completed_bookings)
                
                # Подсчет выручки с бронирований
...
async def cmd_migrate_db(message: types.Message):
    ...
    try:
        if DATABASE_URL:
            await message.answer("⚠️ Миграция не требуется для PostgreSQL.\nБД автоматически создаётся при запуске.")
            return
...
async def unknown_message_debug(message: types.Message, state: FSMContext):
    ...
    if message.text.isdigit():
        ...
        print(f"⚠️ НЕИЗВЕСТНОЕ СООБЩЕНИЕ от {message.from_user.id}: '{message.text}' (состояние: {current_state})")
