...
def format_booking_to_order_status(booking: Any, db) -> OrderStatus:
    ...
    return OrderStatus(
        ...
        pickup_time=str(booking_dict.get('pickup_time')) if booking_dict.get('pickup_time') else None,
...
async def get_order_qr_code(booking_id: int, db=Depends(get_db)):
    ...
    if status not in ['confirmed', 'ready']:
        raise HTTPException(
            status_code=400,
            detail="QR код доступен только для подтвержденных заказов"
        )
