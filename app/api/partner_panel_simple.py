...
from app.services.stats import get_partner_stats, Period, PartnerTotals
...
def verify_telegram_webapp(authorization: str) -> int:
    ...
    For local development, skips verification if authorization starts with 'dev_'
    """
...
async def confirm_order(
    ...
    db_order_type = order.get('order_type') if isinstance(order, dict) else (order[5] if len(order) > 5 else 'delivery')
...
async def upload_photo(
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            form_data.add_field('photo', content, 
