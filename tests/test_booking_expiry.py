...
async def test_booking_expiry_and_reminder(db_instance, monkeypatch):
    ...
    with db.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO bookings (user_id, offer_id, quantity, booking_code, status, expiry_time, reminder_sent) VALUES (?, ?, ?, ?, 'pending', datetime('now', '+70 minutes'), 0)", (user_id, offer_id, 1, 'TEST01'))
    ...
    with db.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT booking_id FROM bookings WHERE reminder_sent = 0 AND expiry_time IS NOT NULL AND expiry_time > datetime('now') AND expiry_time <= datetime('now', '+1 hour')")
    ...
    with db.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("UPDATE bookings SET expiry_time = datetime('now', '+50 minutes') WHERE booking_id = ?", (booking_id,))
        conn.commit()
    ...
    with db.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("UPDATE bookings SET expiry_time = datetime('now', '-1 minute') WHERE booking_id = ?", (booking_id,))
        conn.commit()
