...
class TestInputValidator:
    ...
    def test_validate_phone_invalid_characters(self):
        ...
        assert self.validator.validate_phone("+123-456-7890") is False
        assert self.validator.validate_phone("123 456 7890") is False
        assert self.validator.validate_phone("(123)4567890") is False
...
class TestRateLimiter:
    ...
    def test_is_allowed_different_actions(self):
        ...
        for _ in range(3):
            assert self.limiter.is_allowed(user_id, action1, max_requests=3, window_seconds=60) is True
