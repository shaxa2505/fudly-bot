...
// ONLY allow dev mode on localhost/127.0.0.1 for security
const isLocalhost = window.location.hostname === 'localhost' ||
                   window.location.hostname === '127.0.0.1' ||
                   window.location.hostname.includes('ngrok');
const IS_DEV_MODE = !window.Telegram?.WebApp?.initData && isLocalhost;

let DEV_TELEGRAM_ID = null;
if (IS_DEV_MODE) {
    // Use sessionStorage instead of localStorage for better security
    DEV_TELEGRAM_ID = sessionStorage.getItem('dev_telegram_id');
    if (!DEV_TELEGRAM_ID) {
        ...
        if (DEV_TELEGRAM_ID) {
            ...
            if (!/^\d+$/.test(DEV_TELEGRAM_ID)) {
                ...
                console.log('🔑 Auth mode: Development (ID: ' + DEV_TELEGRAM_ID + ')');
                console.log('💡 Tip: Your Telegram ID is saved in sessionStorage for this session only.');
            }
        }
    } else {
        console.log('🔑 Auth mode: Development (ID: ' + DEV_TELEGRAM_ID + ')');
    ...
    if (!isLocalhost) {
        console.log('🔒 Production mode: Telegram WebApp authentication');
    }
}
...
async function parseErrorMessage(response) {
    try {
        ...
        if (data.detail) {
            return typeof data.detail === 'string' ? data.detail : 'Ошибка сервера';
        }
...
/**
 * Make API request with duplicate prevention
...
function renderProducts(items) {
    ...
    container.innerHTML = items.map(product => {
        ...
        <div class="product-card ${product.status}" data-product-id="${product.offer_id}">
            ...
            <div class="product-image" data-photo-id="${escapeHtml(product.photo_id || '')}"></div>
            <div class="product-info">
                ...
                <div class="product-badges">
                    <span class="badge ${qtyClass}">${qtyIcon} ${product.quantity} ${escapeHtml(product.unit)}</span>
    ...
    container.innerHTML = items.map(order => {
        ...
        <div class="order-card">
            <div class="order-header">
                ...
                <span class="status-badge status-${order.status}">${escapeHtml(getOrderStatusText(order.status))}</span>
            </div>
            <div class="order-items">
                <strong>${escapeHtml(order.offer_title || 'Товар')}</strong> × ${order.quantity}
            </div>
            <div><strong>Тип:</strong> ${typeLabel}</div>
            ${order.delivery_address ? `<div style="font-size:13px;color:var(--hint-color);">📍 ${escapeHtml(order.delivery_address)}</div>` : ''}
            <div><strong>Сумма:</strong> ${order.price} сум</div>
            <div style="font-size:14px;margin-top:8px;">
                👤 Клиент: ${escapeHtml(order.customer_name || 'Неизвестно')}<br>
...
window.confirmOrder = async function(orderId, orderType = 'booking') {
    const result = await makeUniqueRequest(`confirm_${orderId}`, async () => {
        haptic('medium');

        try {
            const data = await apiRequest(`/partner/orders/${orderId}/confirm?order_type=${orderType}`, {
                method: 'POST'
            });
...
window.cancelOrder = async function(orderId, orderType = 'booking') {
    ...
    const result = await makeUniqueRequest(`cancel_${orderId}`, async () => {
        haptic('medium');

        try {
            const data = await apiRequest(`/partner/orders/${orderId}/cancel?order_type=${orderType}`, {
                method: 'POST'
            });
...
window.markReady = async function(orderId, orderType = 'booking') {
    const result = await makeUniqueRequest(`ready_${orderId}`, async () => {
        haptic('medium');

        try {
            const data = await apiRequest(`/partner/orders/${orderId}/status?status=ready&order_type=${orderType}`, {
                method: 'POST'
            });
...
window.markDelivering = async function(orderId, orderType = 'order') {
    const result = await makeUniqueRequest(`delivering_${orderId}`, async () => {
        haptic('medium');

        try {
            const data = await apiRequest(`/partner/orders/${orderId}/status?status=delivering&order_type=${orderType}`, {
                method: 'POST'
            });
