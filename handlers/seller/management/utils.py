...
def get_booking_field(booking: Any, field: str, default: Any = None) -> Any:
    ...
    field_map = {
        "booking_id": 0, "store_id": 1, "offer_id": 2, "user_id": 3,
        "status": 4, "quantity": 5, "first_name": 6, "phone": 7,
        "booking_code": 8, "created_at": 9
    }
    idx = field_map.get(field)
...
async def send_order_card(message: types.Message, order: Any, lang: str, is_booking: bool = True) -> None:
    """Send order/booking card with action buttons."""
    database = get_db()
    
    if is_booking:
        # Booking fields
        booking_id = get_booking_field(order, 'booking_id') or (order[0] if isinstance(order, (list, tuple)) and len(order) > 0 else None)
