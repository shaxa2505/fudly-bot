        ...
        if db and order_id:
            try:
                ...
                if hasattr(db, "get_booking"):
                    booking = db.get_booking(order_id)
                    if booking:
                        offer_id = booking.get("offer_id") if isinstance(booking, dict) else (
                            booking[1] if len(booking) > 1 else None
                        )
                        if offer_id and hasattr(db, "get_offer"):
                            offer = db.get_offer(offer_id)
                            if offer:
                                store_id = offer.get("store_id") if isinstance(offer, dict) else (
                                ...
                                if store_id and hasattr(db, "get_store_owner"):
                                    ...
                                    if owner_id:
                                        ...
                                        title = offer.get("title") if isinstance(offer, dict) else (
                                            offer[2] if len(offer) > 2 else "Товар"
                                        )
