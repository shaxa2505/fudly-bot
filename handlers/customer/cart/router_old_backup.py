...
async def checkout_pickup(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    for store_id, store_bookings in stores_bookings.items():
        try:
            ...
            kb = InlineKeyboardBuilder()
            confirm_text = (
                "✅ Barchasini qabul qilish" if seller_lang == "uz" else "✅ Подтвердить все"
            )
            ...
            logger.info(
                f"Sent batch booking notification to seller {owner_id} for bookings {booking_ids}"
            )
...
async def finalize_cart_order(
    ...
    for store_id, store_orders in stores_orders.items():
        try:
            ...
            reject_text = "❌ Barchasini rad etish" if seller_lang == "uz" else "❌ Отклонить все"
            kb.button(
                text=confirm_text, callback_data=f"partner_confirm_order_batch_{order_ids_str}"
            )
            ...
            logger.info(
                f"Sent batch order notification to seller {owner_id} for orders {order_ids}"
            )
