...
async def repeat_order(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if hasattr(order, "get"):
        ...
        quantity = float(order.get("quantity", 1) or 1)
        store_id = order.get("store_id")
        title = order.get("title", "Товар")
        price = order.get("discount_price", 0)
        available = float(order.get("available_qty", 0) or 0)
        ...
        offer_id = order[2]
        quantity = float(order[3]) if len(order) > 3 and order[3] is not None else 1.0
        store_id = order[4]
        title = order[5] if len(order) > 5 else "Товар"
        price = order[6] if len(order) > 6 else 0
        available = float(order[7]) if len(order) > 7 and order[7] is not None else 0.0
            ...
            for item in cart_items:
                ...
                if offer and offer.get("status") == "active":
                    ...
                    cart_storage.add_item(
                        ...
                        max_quantity=float(get_offer_field(offer, "quantity", 1) or 1),
