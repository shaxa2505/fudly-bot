...
async def admin_analytics(message: types.Message):
    ...
    try:
        ...
        avg_discount = round(avg_discount_result[0], 1) if avg_discount_result and avg_discount_result[0] else 0
...
🏆 <b>ТОП-5 ПРОДАВЦОВ:</b>"""
        ...
        await message.answer_document(
            csv_file,
            caption="📊 Полная аналитика в CSV формате"
        )
...
"""
            ...
            await message.answer(
                store_text,
                reply_markup=_moderation_keyboard(store['store_id']),
                parse_mode="HTML"
            )
